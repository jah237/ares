...
ips_ex4 <- function(L,id){
  ...
  name <- paste(paste("/storage/u1664122/results9-11/task",id,L,sep="_"),".RData",sep="")
  save(out, file=name)
...
ips_ex6 <- function(L,id){
  ...
  name <- paste(paste("OU-1point5-check/task",id,sep="_"),".RData",sep="")
  save(out, file=name)
...
ips_ex7 <- function(id){
  ...
  lambda <- function(N){
    #out <- rnorm(N,0,3/sqrt(2))
    out <- rgamma(N,shape=2*theta*mu/sigma^2,rate=2*theta/sigma^2)
    return(out)
  }
  ...
  K <- function(x){
    ...
    for(i in 1:n_steps){
      W <- rnorm(N); U <- rnorm(N)
      delta_W <- sqrt(delta)*W
      delta_Z <- 1/2 * delta^(3/2) * (W + 1/sqrt(3) * U)
      x <- x + theta*(mu-x)*delta + sigma*sqrt(abs(x))*delta_W
           + sigma^2/4 * sign(x)* (delta_W^2 - delta)
           - theta*sigma*sqrt(abs(x))*delta_Z
           - theta^2/2 * (mu-x) * delta^2
           + theta*sigma/2 * (mu-x)/sqrt(abs(x)) * (sign(x)-sigma^2/4) * (delta*delta_W - delta_Z)
    }
  ...
  K_uncoupled <- function(x1,x2){
    ...
    for(i in 1:n_steps){
      ...
      sgn <- (sgn + 1)%%2
      W1 <- rnorm(N); U1 <- rnorm(N)
      x1 <- K_step(delta,x1,W1,U1)

      if(sgn==1){
        W2 <- rnorm(N); U2 <- rnorm(N)
        x2 <- K_step(2*delta,x2,W2,U2)
      }
    }
  ...
  name <- paste(paste("full-results/task",id,sep="_"),".RData",sep="")
  save(out, file=name)
